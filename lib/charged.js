 ...
 * http://docs.chargify.com/api-subscriptions
 *
 * Chargify paginates the response for the list query below.
 *
 * By default the page size is 20 and it goes upto a max of 200
 * This presents an issue -
 *    that we need to iterate through each page and
 *    concatenate the result at each iteration.
 *
 * We could possibly use a standard nodejs library - such as async
 * to do the above for us, however, chargify does not provide the
 * page information in the response. THe only way to figure out that
 * we have reached the end of the line is to see thta the response is
 * null.
 * Hence, in the function below, we resursively iterate through ,
 * each time calling the get with a diff page # , until the response
 * is null.
 */
...
Charged.prototype.listSubscriptions = function (options, callback) {
	...
	var results = []; // the container for the concatenated result

	var nextPage = function () {
		options.page = page;
		options.per_page = 200; // max allowed number per page
		scope.listSubscriptionsPage(options, function (err, subscriptions, obj) {
			...
			if (subscriptions.length) {
				results = results.concat(subscriptions);
				if (subscriptions.length === 200) {
					page++;
					nextPage(); // fetch the next page
					return;
				}
			}
			// we got empty result or last result length was less then 200
...
Charged.prototype.listSubscriptionsPage = function (options, callback) {
	if (!callback) {
		callback = options;
		options = null;
	}
...
['subscription', 'customer', 'component'].forEach(function (resource) {
	['list', 'get', 'create', 'remove'].forEach(function (action) {
		...
		method = action + resource[0].toUpperCase() + resource.substring(1) + 'Metafields';
...
Charged.prototype.getRelevantCustomerDetails = function (subscriberDetails, customerDataFromRemoteProvider) {
	...
	if (typeof subscriberDetails === "object") {
		for (var item in subscriberDetails) {
			...
			var currentBillingCycleEndDate = subscriberDetails[item].expires_at ? subscriberDetails[item].expires_at : subscriberDetails[item].current_period_ends_at;
			var billingStatus = translateChargifySubscriptionState(billingStatusRaw);
			var billingSubscriptionId = subscriberDetails[item].id;
			var billingSubscriptionPaymentMethods = subscriberDetails[item].credit_card ? [subscriberDetails[item].credit_card] : [];
			var billingProductId = null;
			var billingProductSku = null;
			var billingProductHandle = null;
			var billingProductFamily = null;
			var billingProductName = null;

			//Max. number of tokens will be deduced from Product Handle naming: ...-<MAX_Num>-product-apihandle
			var maxNumOfTokensExtractedFromProducthandle = 0;
			if (subscriberDetails[item].product) {
				billingProductId = subscriberDetails[item].product.id;
				billingProductSku = subscriberDetails[item].product.accounting_code;
				billingProductHandle = subscriberDetails[item].product.handle;
				billingProductFamily = subscriberDetails[item].product.product_family.name;
				billingProductName = subscriberDetails[item].product.name;
				if (billingProductFamily.toLowerCase() == "discovery") {
					billingProductFamily = "CWDiscovery";
				}
				if (billingProductHandle) {
					var billingProducthandleSplit = billingProductHandle.split("-");
					if (billingProducthandleSplit.length >= 4) {
						var numberOfTokensInt = parseInt(billingProducthandleSplit[billingProducthandleSplit.length - 3]);
						if (!isNaN(numberOfTokensInt)) {
							maxNumOfTokensExtractedFromProducthandle = numberOfTokensInt;
						}
					}
				}
			}

			customerDataFromRemoteProvider[item] = {
				"customerID": customerID,
				"billingStatus": billingStatus,
				"currentBillingCycleStartDate": currentBillingCycleStartDate,
				"currentBillingCycleEndDate": currentBillingCycleEndDate,
				"billingStatusRaw": billingStatusRaw,
				"billingSubscriptionId": billingSubscriptionId,
				"billingProduct": {
					"id": billingProductId,
					"sku": billingProductSku,
					"name": billingProductName,
					"family": billingProductFamily,
					"handle": billingProductHandle,
					"maxTokens": maxNumOfTokensExtractedFromProducthandle
				},
				"billingSubscriptionPaymentMethods": billingSubscriptionPaymentMethods
			};
		}
	}
...
translateChargifySubscriptionState = function (billingStatusRaw) {
	var billingStatus;
